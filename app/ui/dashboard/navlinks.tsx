...
import { LayoutDashboard, Users, ShoppingBag, UserCircle, Settings, Package, Layers, AlertTriangle, BarChart3 } from "lucide-react";
...
const navItems = [
  ...
  { icon: BarChart3, label: "Inventory Report", path: "/dashboard/inventory-report" },
  { icon: Settings, label: "Settings", path: "/dashboard/settings" },
];

export default function NavLinks({ role }: NavLinksProps) {
  const pathname = usePathname();
  console.log('NavLinks - User role:', role);
  return (
    <nav className=" space-y-2 flex-grow">
      {navItems.map((item) => {
        const IconComponent = item.icon;
        const isActive = pathname === item.path;
        const hasAccessToItem = role && hasAccess(role, item.path);
        console.log(`Route ${item.path}: hasAccess=${hasAccessToItem}`);
        if (hasAccessToItem) {
          return (
            <Link
              key={item.label}
